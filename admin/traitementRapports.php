...
//Upload de rapport (récupération des infos et traitement)
if (isset($_REQUEST["action"])){
	if($_REQUEST["action"]=="upload"){
		if(is_uploaded_file($_FILES['f_pdf']['tmp_name'])){
			//on verifie que le fichier envoyé est bien au format pdf
			
			$type = $_FILES['f_pdf']['name'];
			
			if(!strstr($type, 'pdf')){
			  echo "<br /><br /><span><b>ERREUR: Le rapport doit être au format pdf.</b></span><br />";
			  echo "<span><a href=\"javascript:history.back()\">Retour</a></span>";
			  exit();
			}	
			else{		
	...
	if($_REQUEST["action"]=="conf_modif"){
		if (isset($_REQUEST["id"])){
			...
			if ($_REQUEST["n_rapport"] ==""){
				echo "<br /><br /><span><b>Erreur: le nom du rapport ne peut pas être null.</b></span><br />";
				echo "<span><a href=\"javascript:history.back()\">Retour</a></span>";
				exit();			
			}
