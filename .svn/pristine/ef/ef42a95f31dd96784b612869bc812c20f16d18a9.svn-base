	...
	if($_REQUEST["action"]=="upload"){
		...
		if(isset($erreur) && ($erreur=='trouve')){
			...
			exit();
				...
				$d_upload = date("Y-m-d");
				$type_stage_prop="";
				$sql = "INSERT INTO propositions VALUES (\"NULL\",\"$n_file\",\"$n_proposition\",\"$d_upload\",0,\"$resume\",\"$id_entp\",\"$email\",\"$type_stage_prop\",\"En attente de validation\",NULL,NULL);";
				mysql_query($sql);
				...
				$sql = 'SELECT n_proposition, mail_resp_prop_pdf, raisonsociale_entreprise FROM propositions,entreprise WHERE propositions.id_proposition_pdf='.$id_prop.' AND entreprise.id_entreprise=propositions.id_entreprise limit 1';
				$resultat =  mysql_query($sql) or die ('Erreur : '.mysql_error() );
				while ($propositions = mysql_fetch_array($resultat, MYSQL_ASSOC))
				{
					$raisonsociale_entreprise = $propositions['raisonsociale_entreprise'];
					$n_proposition = $propositions['n_proposition'];
				}
				
				$m = new Mailer('/mailTemplates/notificationAdminAjoutPropositionPdf');
				$m->fillTemplateVar("raison_sociale_etp", $raisonsociale_entreprise);
				$m->fillTemplateVar("intitule_proposition",$n_proposition);

				$m->sendMail(DBConfig::getConfigValue("MAIL ADMINISTRATEURS"), "stages_miage@miage.emi.u-bordeaux1.fr", DBConfig::getConfigValue("MAIL ADMINISTRATEURS"), "Site MIAGe des stages : Ajout d'une proposition PDF");
